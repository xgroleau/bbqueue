...
mod tests {
    ...
    use bbqueue::{
        BBQueue, Consumer, GrantR, GrantW, Producer, StaticStorageProvider, StorageProvider,
    };
    ...
    static mut BB: BBQueue<StaticStorageProvider<BUFFER_SIZE>> = BBQueue::new_static();
    ...
    fn hello() {
        ...
        // create the channels
        let (tx_1_2, rx_1_2): (
            Sender<Potato<'static, StaticStorageProvider<BUFFER_SIZE>>>,
            Receiver<Potato<'static, StaticStorageProvider<BUFFER_SIZE>>>,
        ) = channel();
        let (tx_2_3, rx_2_3): (
            Sender<Potato<'static, StaticStorageProvider<BUFFER_SIZE>>>,
            Receiver<Potato<'static, StaticStorageProvider<BUFFER_SIZE>>>,
        ...
        let thread_1 = spawn(move || {
            ...
            let mut me: Potato<'static, StaticStorageProvider<BUFFER_SIZE>> = Potato::Idle;
        ...
        let closure_2_3_4 =
            move |rx: Receiver<Potato<'static, StaticStorageProvider<BUFFER_SIZE>>>,
                  tx: Sender<Potato<'static, StaticStorageProvider<BUFFER_SIZE>>>| {
                let mut me: Potato<'static, StaticStorageProvider<BUFFER_SIZE>> = Potato::Idle;
                let mut count = 0;
